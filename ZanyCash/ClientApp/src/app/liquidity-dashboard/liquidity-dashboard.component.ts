  ...
  ngOnInit(): void {
    ...
    this.transactions$ = combineLatest([this.data.liquidity$, selectedDate$]).pipe(
      map(([liquidities, date]) => {
        const currentLiquidity = liquidities.find(l => new Date(l.date).getTime() === new Date(date + 'T00:00:00').getTime());
        return currentLiquidity? currentLiquidity.transactions : [];
      })
    );
